...
class wpl_notifications
{
    ...
    public function send()
    {
        ...
        if($this->mode == 'email')
        {
            ...
            foreach($this->recipients as $recipient)
            {
                ...
                if(isset($sent[$mail_to])) continue;
        ...
        elseif($this->mode == 'sms')
        {
            ...
            foreach($this->recipients as $recipient)
            {
                ...
                {
                    $sms_to = $recipient;
                    $user_id = $this->sms->wpl_get_user_data('`wpl`.`id`', wpl_db::prepare('AND `wpl`.`mobile` = %s', $sms_to), 'loadResult');
                }

                if(isset($sent[$sms_to])) continue;
    ...
    public static function get_notification($id)
    {
        ...
        return wpl_db::select(wpl_db::prepare("SELECT * FROM `#__wpl_notifications` WHERE `id` = %d", $id), 'loadAssoc');
    }
     ...
     * @return string
     */
    public static function get_template_path($path, $override = true)
    {
        // First Validation
        ...
        if($override){
            $override_tpl = str_replace('libraries' . DS, 'overrides' . DS . 'libraries' . DS, $tpl);
            if(wpl_file::exists($override_tpl)) {
                $tpl = $override_tpl;
            }
        }
    ...
    public function set_recipients($recipients)
    {
        if($this->mode == 'email')
        {
            ...
            foreach($recipients as $recipient)
            {
                ...
                {
                    $users = wpl_users::get_wpl_users(wpl_db::prepare("AND `membership_id` = %s", $recipient));
        ...
        elseif($this->mode == 'sms')
        {
            ...
            foreach($recipients as $recipient)
            {
                ...
                {
                    $user_data = $this->sms->wpl_get_user_data('`u`.`ID`,`wpl`.`mobile`', wpl_db::prepare("AND wpl.`id` = %d", $recipient), 'loadObject');
                ...
                elseif(is_numeric($recipient) and $recipient < 0)
                {
                    $users = $this->sms->wpl_get_user_data('`u`.`ID`,`wpl`.`mobile`', wpl_db::prepare("AND `membership_id` = %s", $recipient), 'loadObjectList');
                ...
                {
                    $user_id = $this->sms->wpl_get_user_data('`wpl`.`id`', wpl_db::prepare('AND `wpl`.`mobile` = %s', $recipient), 'loadResult');
                    if(!$user_id) $user_id = 0;
    ...
    {
        return wpl_db::select("SELECT * FROM `#__wpl_notifications` WHERE 1 " . $where, $result);
    }
    ...
    public static function save_notification($data)
    {
		...
		$values = ['template' => $data['template_path']];
        /** Get SMS value */
        if(wpl_global::check_addon('sms'))
        {
			...
			$values['sms_additional_memberships'] = $data['include_sms_membership'];
			$values['sms_additional_users'] = $data['include_sms_user'];
			$values['sms_additional_mobile'] = $data['include_sms_mobile'];
        }
		...
		$values['additional_emails'] = $data['include_email'];
		$values['additional_memberships'] = $data['include_membership'];
		$values['additional_users'] = $data['include_user'];
		$values[$subject_column] = $data['subject'];
        return wpl_db::update('wpl_notifications', $values, 'id', $data['id']);
    }
