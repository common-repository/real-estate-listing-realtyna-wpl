...
class wpl_render
{
	 ...
	 * @return string[]
	 */
	public static function render_url($url, $split = ',')
    {
		$split = preg_quote($split, "'");

		$length = preg_match_all("' 
 			( (?:https?\://)? [^$split]+ ) # capture everything until finding first \$split character
 			 $split?  						# escape  \$split character
 			  \s* ( https?\:// )?           # capture next url protocol 
 			 'ix", $url, $match);

		if($length < 2) return array($url);
		...
		for($i = 0; $i < $length; $i ++)
		{
			if($i)
			{
				$protocol = $match[2][ $i - 1 ];
				$urls[]   = $protocol . $match[1][ $i ];
	...
	public static function render_price($price, $unit_id = NULL, $symbol = '', $price_text = '')
	{
        ...
        if(!trim($symbol ?? '')) $symbol =wpl_esc::return_t(@$currency['name']);
		$decimal = 2;
	...
	public static function derender_date($date)
	{
		...
		if(empty($date_parts) || count($date_parts) == 1) {
			return $date;
		}
		$standard_date = array();
