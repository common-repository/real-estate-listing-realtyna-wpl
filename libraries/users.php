...
class wpl_users
{
	...
	public static function delete_user_from_wpl($user_id)
	{
		...
		return wpl_db::delete('wpl_users', $user_id);
	}
	...
	public static function add_user_to_wpl($user_id, $group_id = -1)
	{
		...
		if($user_data)
		{
			$values['first_name'] = $user_data->data->meta['first_name'];
			$values['last_name'] = $user_data->data->meta['last_name'];
			$values['main_email'] = $user_data->data->user_email;
		}
		
		$result = wpl_db::insert('wpl_users', $values);
	...
	{
		return wpl_db::select("SELECT * FROM `#__users` AS u LEFT JOIN `#__wpl_users` AS wpl ON u.ID = wpl.id WHERE 1 $condition");
	}
	...
	{
		return wpl_db::select("SELECT * FROM `#__users` AS u INNER JOIN `#__wpl_users` AS wpl ON u.ID = wpl.id WHERE 1 $condition");
	}
	...
	public static function get_id_by_username($username)
	{
		...
		$user = wpl_db::select(wpl_db::prepare('SELECT * FROM `#__users` WHERE `user_login` = %s', $username), 'loadObject');
	...
	public static function get_id_by_email($email)
	{
		...
		$user = wpl_db::select(wpl_db::prepare('SELECT * FROM `#__users` WHERE `user_email` = %s', $email), 'loadObject');
     ...
     * @return array of objects
     */
	public static function get_wpl_memberships($condition = '')
	{
		return wpl_db::select("SELECT * FROM `#__wpl_users` WHERE 1 AND `id` < 0 ".(trim($condition ?? '') ? $condition : '')." ORDER BY `index` ASC");
	}
    ...
    {
        return wpl_db::select(wpl_db::prepare('SELECT * FROM `#__wpl_user_group_types` WHERE `enabled` >= %d ORDER BY `index` ASC', $enabled), $return_type);
    }
    ...
    {
        return wpl_db::select(wpl_db::prepare('SELECT * FROM `#__wpl_user_group_types` WHERE `id` = %d AND `enabled` >= %d', $id, $enabled), 'loadObject');
    }
    ...
    public static function create_default_user_type()
    {
        ...
        wpl_db::insert('wpl_user_group_types', [
			'id' => $id,
			'editable' => 2,
			'deletable' => 1,
			'index' => $id,
    ...
    {
        $result = wpl_db::select("SELECT MAX(id) as max_id FROM `#__wpl_user_group_types`", 'loadResult');
		$id = max($result, 100);
    ...
    public static function remove_user_type($id)
	{
		...
		wpl_db::delete('wpl_user_group_types', $id);
		wpl_db::set('wpl_users', $id, 'membership_type', '', 'membership_type');
	...
	{
		$result = wpl_db::select("SELECT MIN(id) as min_id FROM `#__wpl_users`", 'loadResult');
	...
	public static function remove_membership($membership_id)
	{
		...
		wpl_db::set('wpl_users', $membership_id, 'membership_id', -1, 'membership_id');
		
        /** trigger event **/
		wpl_global::event_handler('membership_removed', array('id'=>$membership_id));
		
		wpl_db::delete('wpl_users', $membership_id);
	...
	public static function get_membership($membership_id)
	{
		...
		return wpl_db::select(wpl_db::prepare("SELECT * FROM `#__wpl_users` WHERE `id` = %d", $membership_id), 'loadObject');
	}
     ...
     * @param string $method
     */
	public static function change_membership($user_id, $membership_id = -1, $trigger_event = true, $method = '')
	{
		$user_data = wpl_users::get_wpl_data($user_id);
		...
		$data['membership_type'] = $membership_data->membership_type;

		wpl_db::update('wpl_users', $data, 'id', $user_id);
	...
	public static function get_users_properties_count($user_id = '', $condition = '')
	{
        ...
        if(wpl_users::is_broker($user_id))
        {
            ...
            return wpl_db::select("SELECT COUNT(id) FROM `#__wpl_properties` WHERE `user_id` IN (".implode(',', $user_ids).") ".$condition, 'loadResult');
        }
		return wpl_db::select( wpl_db::prepare("SELECT COUNT(id) FROM `#__wpl_properties` WHERE `user_id` = %d " . $condition, $user_id), 'loadResult');
	}
    ...
    {
        return wpl_db::select("SELECT COUNT(*) AS count FROM `#__wpl_users` WHERE 1 " . $this->where, 'loadResult');
    }
	...
	public static function finalize($user_id)
	{
		...
		if(wpl_global::check_addon('membership')) {
			wpl_db::set('wpl_users', $user_id, 'maccess_fpc', 0);
		}
	...
	public static function update_text_search_field($user_id)
	{
		...
		wpl_db::set('wpl_users', $user_id, $column, implode(' ', $text_search_data));
    }
	...
	public static function generate_rendered_data($user_id)
	{
		...
		wpl_db::set('wpl_users', $user_id, $column, $result);
	...
	public static function generate_location_text($user_data, $user_id = 0, $glue = ',')
	{
        ...
        if( trim($user_data['location7_name'] ?? '') != '') $locations['location7_name'] = wpl_esc::return_html_t($user_data['location7_name']);
        if( trim($user_data['location6_name'] ?? '') != '') $locations['location6_name'] = wpl_esc::return_html_t($user_data['location6_name']);
		if( trim($user_data['location5_name'] ?? '') != '') $locations['location5_name'] = wpl_esc::return_html_t($user_data['location5_name']);
        if( trim($user_data['location4_name'] ?? '') != '') $locations['location4_name'] = wpl_esc::return_html_t($user_data['location4_name']);
        if( trim($user_data['location3_name'] ?? '') != '') $locations['location3_name'] = wpl_esc::return_html_t($user_data['location3_name']);
        if( trim($user_data['location2_name'] ?? '' ) != '') $locations['location2_name'] = wpl_esc::return_html_t($user_data['location2_name']);
        if( trim($user_data['location1_name'] ?? '') != '') $locations['location1_name'] = wpl_esc::return_html_t($user_data['location1_name']);
        if( trim($user_data['zip_name'] ?? '') != '') $locations['zip_name'] = wpl_esc::return_html_t($user_data['zip_name']);

        // Location Abbr Names
        if( trim($user_data['location1_name'] ?? '') ) $locations['location1_abbr'] = wpl_esc::return_html_t(wpl_locations::get_location_abbr_by_name($user_data['location1_name'], 1));
        if( trim($user_data['location2_name'] ?? '') ) $locations['location2_abbr'] = wpl_esc::return_html_t(wpl_locations::get_location_abbr_by_name($user_data['location2_name'], 2));
        ...
        /** update **/
		wpl_db::set('wpl_users', $user_id, $column, $location_text);
	...
	public static function insert_user($user_data)
	{
		...
		{
			return new WP_Error('broke', wpl_esc::return_html_t("ERROR: Required fileds are invalid!"));
		}
	}
	...
	public static function login_user($user_data)
	{
		...
		{
			return new WP_Error('broke', wpl_esc::return_html_t('ERROR: Required fields are invalid!'));
		}
	}
    ...
    public static function validate_activation_key($key)
    {
        ...
        $id = wpl_db::select(wpl_db::prepare('SELECT `id` FROM `#__users` WHERE `user_activation_key` = %s', $key), 'loadResult');
     ...
     * @return string
     */
    public static function get_user_type_tpl($wplpath, $tpl = '', $user_type = NULL)
	{
    ...
    public static function save_default_values($user_id, $values = array())
    {
		...
		return wpl_db::set('wpl_users', $user_id, 'default_values', json_encode($values));
    }
