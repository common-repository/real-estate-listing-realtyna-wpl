...
class wpl_request
{
	...
	public static function getVar($name, $default = null, $hash = 'default', $clean = false)
	{
		...
		$var = is_array($input) && array_key_exists($name, $input) ? $input[$name] : $default;

		// Fix a Vulnerability
		if(in_array($name, array('wplview', 'alias', 'pid'))) $var = htmlentities($var ?? "");
...
class wpl_download
{
    ...
    public function add($url)
    {
        ...
        $ch = apply_filters('wpl_download/add/curl', $ch);
