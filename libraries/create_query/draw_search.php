...
if($format == 'radiussearchunit' and !$done_this)
{
	...
	if($latitude and $longitude and $radius and $unit_id)
	{
		...
		if($unit)
		{
			...
			$query .= wpl_db::prepare(' AND (( %f * acos( cos( radians(%f) ) * cos( radians( googlemap_lt ) ) * cos( radians( googlemap_ln ) - radians(%f) ) + sin( radians(%f) ) * sin( radians( googlemap_lt ) ) ) ) < %f) AND `show_address` = 1', $tosi, $latitude, $longitude, $latitude, $radius);
		}
	}
...
elseif(($format == 'polygonsearch' or $format == 'dmgfc') and !$done_this)
{
    ...
    foreach($polygons as $polygon)
    {
        ...
        $qq[] = $sql_function . wpl_db::prepare("(ST_GeomFromText(%s), geopoints) = 1", "Polygon(($polygon_str))");
    }
    ...
    foreach($exp_location_data as $key=>$value)
    {
        ...
        if($latitude and $longitude and $radius and $unit_id)
        {
            ...
            if($unit)
            {
                ...
                $queries[] = wpl_db::prepare(' ((%f * acos( cos( radians(%f) ) * cos( radians( googlemap_lt ) ) * cos( radians( googlemap_ln ) - radians(%f) ) + sin( radians(%f) ) * sin( radians( googlemap_lt ) ) ) ) < %f) ', $tosi, $latitude, $longitude, $latitude, $radius);
            }
        }
    }
