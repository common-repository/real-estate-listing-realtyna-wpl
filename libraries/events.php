...
class wpl_events
{
	...
	public static function get_events($trigger, $enabled = 1)
	{
		...
		$results = wpl_db::select(wpl_db::prepare("SELECT * FROM `#__wpl_events` WHERE `enabled` >= %d ORDER BY `id` ASC", $enabled));
	...
	public static function do_cronjobs($params = array())
	{
		...
		foreach($cronjobs as $cronjob)
		{
			...
			/** call function **/
			if(class_exists($cronjob->class_name)) {
				$cron_obj = new $cronjob->class_name();
				if (method_exists($cron_obj, $cronjob->function_name)) {
