...
{
    /**
     * PHP scss compiler instance
     * @var object
     */
    public $scssc;
    
    /**
     * Full path to SCSS path
     * @var string
     */
    public $import_path;
     ...
     * @var string
     */
    public $formatter = 'wpl_scss_formatter_compressed';
    ...
    {
        $this->scssc = new wpl_scssc();
        
        $this->set_import_path(WPL_ABSPATH.'assets'.DS.'scss'.DS);
        $this->set_formatter($this->formatter);
    }
    ...
    {
        /** Add WPL Prefix to formatter **/
        if(strpos($formatter ?? '', 'wpl_') === false) $formatter = 'wpl_'.$formatter;
    ...
    public function set_import_path($path)
    {
        ...
        $this->scssc->setImportPaths($this->import_path);
    }
    
    /**
     * Compile SCSS code and convert it to CSS code
     * @author Howard <howard@realtyna.com>
    ...
    {
        return $this->scssc->compile($code, $name);
    }
    ...
    {
        if(!trim($input ?? '') or !wpl_file::exists($input)) return false;
 ...
 * by `wpl_scss_parser` into a syntax tree, then it is compiled into another tree
 * representing the CSS structure by `wpl_scssc`. The CSS tree is fed into a
 * formatter, like `wpl_scss_formatter` which then outputs CSS as a string.
 *
 * During the first compile, all values are *reduced*, which means that their
 * types are brought to the lowest form before being dump as strings. This
 * handles math equations, variable dereferences, and the like.
 *
 * The `parse` function of `wpl_scssc` is the entry point.
 *
 * In summary:
 *
 * The `wpl_scssc` class creates an instance of the parser, feeds it SCSS code,
 * then transforms the resulting tree to a CSS tree. This class also holds the
 * evaluation context, such as all available mixins and variables at any given
 * time.
 *
 * The `wpl_scss_parser` class is only concerned with parsing its input.
 *
 * The `wpl_scss_formatter` takes a CSS tree, and dumps it to a formatted string,
...
class wpl_scssc {
	...
	protected $formatter = "wpl_scss_formatter_nested";
	...
	public function compile($code, $name = null)
	{
		...
		$this->parser = new wpl_scss_parser($name);
	...
	protected function injectVariables(array $args)
	{
		...
		$parser = new wpl_scss_parser(__METHOD__, false);
	...
	protected function importFile($path, $out) {
		...
		if (isset($this->importCache[$realPath])) {
			...
			$parser = new wpl_scss_parser($path, false);
...
class wpl_scss_parser {
	...
	static protected function makeOperatorStr($operators) {
		return '('.implode('|', array_map(array('wpl_scss_parser','preg_quote'),
			$operators)).')';
	}
	 ...
	 *
	 * Consider the function wpl_scssc::keyword(). (All parse functions are
	 * structured the same.)
	 *
	 ...
	 *
	 * All of these parse functions are powered by wpl_scssc::match(), which behaves
	 ...
	 * avoid this, wpl_scssc::seek() is used to remember and set buffer positions.
	 *
...
class wpl_scss_formatter {
	...
	protected function block($block) {
		...
		if (!empty($block->lines)) {
			...
			wpl_esc::e($inner . implode($glue, $block->lines));
			if (!empty($block->children)) {
				wpl_esc::e($this->break);
			}
		}
		...
		if (!empty($block->selectors)) {
			$this->indentLevel--;
			if (empty($block->children)) wpl_esc::e($this->break);
			wpl_esc::e($pre . $this->close . $this->break);
		}
	}
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class wpl_scss_formatter_nested extends wpl_scss_formatter {
	public $close = " }";
 ...
 * @author Leaf Corcoran <leafot@gmail.com>
 */
class wpl_scss_formatter_compressed extends wpl_scss_formatter {
	public $open = "{";
	public $tagSeparator = ",";
	public $assignSeparator = ":";
	public $break = "";
...
class wpl_scss_server {
	...
	public function serve($salt = '') {
		...
		if ($input = $this->findInput()) {
			...
			if ($this->needsCompile($input, $output)) {
				try {
					...
					wpl_esc::e('Parse error: ' . $e->getMessage() . "\n");
				}
			}
			...
			wpl_esc::e(file_get_contents($output));
		...
		$v = wpl_scssc::$VERSION;
		wpl_esc::e("/* INPUT NOT FOUND scss $v */\n");
	}
	 ...
	 * @param \wpl_scssc|null $scss     SCSS compiler instance
	 */
