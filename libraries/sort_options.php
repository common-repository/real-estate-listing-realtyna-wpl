...
class wpl_sort_options
{
	...
	public static function get_sort_options($kind = '', $enabled = 1, $condition = '', $output_type = 'loadAssocList', $format_kinds = false)
	{
		...
		if(trim($condition ?? '') == '')
		{
			...
			if(trim($kind ?? '') != '') $condition .= wpl_db::prepare(" AND `kind` LIKE %s", wpl_db::esc_like("[$kind]"));
			$condition .= " ORDER BY `index` ASC";
		}
		
		$result = wpl_db::select("SELECT * FROM `#__wpl_sort_options` WHERE 1 ".$condition, $output_type);
	...
	*/
	public static function add_sort_option($dbst_id, $kind)
	{
		...
		$sort_option = self::get_sort_options('', 0, wpl_db::prepare(" AND `field_name` = %s", "p.{$dbst_field->table_column}"), 'loadObject');
		...
		$result = wpl_db::insert( 'wpl_sort_options', [
			'id' => $insert_id,
			'kind' => $kind_formatted,
			'name' => $dbst_field->name,
			'field_name' => "p.{$dbst_field->table_column}",
			'enabled' => 1,
			'index' => '99.00',
		]);
        ...
        foreach($sort_options as $sort_option)
        {
            if($sort_option['field_name'] == 'ptype_adv')
            {
                ...
                foreach($types as $type)
                {
                    ...
                    $sort_option['name'] =wpl_esc::return_t(wpl_global::pluralize(2, $type['name']));
