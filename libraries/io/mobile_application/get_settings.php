...
class wpl_io_cmd_get_settings extends wpl_io_cmd_base
{
    ...
    private function prepare_array($objects)
    {
        ...
        {
            $array[] = array('value' => $obj['id'], 'text' =>wpl_esc::return_t($obj['name']));
        }
    ...
    {
        $array = array();
        $fields = wpl_flex::get_fields('', 0, 0, '', '', wpl_db::prepare("AND `category` = %s AND `type` = %s AND `kind` = 0 AND `enabled` >= 1", $category, $type));

        foreach ($fields as $field) 
        {
            $array[] = array('column_name' => 'sf_select_'.$field->table_column, 'value' =>wpl_esc::return_t($field->name));
        }
    ...
    private function get_listing_types()
    {
        ...
        {
            $name =wpl_esc::return_t($list['name']);
            $listings[] = array(
                'id' => $list['id'],
                'name' => $name,
                'selected' => $selected,
    ...
    private function get_listing_sorts()
    {
        ...
        {
            $array[] = array('id' => $sort['id'], 'language_keyword' =>wpl_esc::return_t($sort['name']), 'column_name' => $sort['field_name']);
        }
