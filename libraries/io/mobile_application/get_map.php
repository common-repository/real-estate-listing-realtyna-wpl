...
class wpl_io_cmd_get_map extends wpl_io_cmd_base
{
    ...
    private function create_property_show_section($property)
    {
        ...
        {
            array_push($tags,wpl_esc::return_t("Featured"));
        }

        if(!empty($property['sp_hot']) && $property['sp_hot'] != 0)
        {
            array_push($tags,wpl_esc::return_t("Hot"));
        }

        if(!empty($property['sp_openhouse']) && $property['sp_openhouse'] != 0)
        {
            array_push($tags,wpl_esc::return_t("OpenHouse"));
        }

        if(!empty($property['sp_forclosure']) && $property['sp_forclosure'] != 0)
        {
            array_push($tags,wpl_esc::return_t("Forclosure"));
        }
    ...
    public function bubbles_search($where, $number, $zoom_level)
    {
        ...
        $query = wpl_db::prepare('SELECT COUNT(p.id) AS `count`, AVG(p.googlemap_lt) AS `lat`, AVG(p.googlemap_ln) AS `lng`, FORMAT(p.googlemap_lt * %f, %f) AS `g_lt`, FORMAT(p.googlemap_ln * %f, %f) AS `g_ln`', $multiply_number, $number, $multiply_number, $number);
