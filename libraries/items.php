...
class wpl_items
{
	...
	public static function get_items($parent_id, $item_type = '', $parent_kind = 0, $category = '', $enabled = 1, $condition = '', $return_records = false)
	{
		if(wpl_settings::is_mls_on_the_fly() && $parent_kind == 0) {
			...
			if(!empty($media)) {
				foreach ($media as $media_item) {
					...
					$records[] = (object)[
						...
						'item_type' => 'gallery',
						'item_cat' => 'external',
						'item_name' => 'external_image-' . $item_key,
						'creation_date' => date('Y-m-d H:i:s'),
						'edit_date' => date('Y-m-d H:i:s'),
						'enabled' => 1,
						'item_extra1' => null,
						'item_extra2' => null,
						'item_extra3' => $media_item['MediaURL'],
						'item_extra4' => 'rf',
						'item_extra5' => $media_item['Thumbnail'],
						'params' => null,
						'index' => $item_key,
					];
				}
			...
			if(trim($condition ?? '') == '')
			{
				$condition = wpl_db::prepare(" AND `parent_id` = %d AND `parent_kind` = %d", $parent_id, $parent_kind);

				if(trim($item_type ?? '') != '') $condition .= wpl_db::prepare(" AND `item_type` = %s", $item_type);
				if(trim($category ?? '') != '') $condition .= wpl_db::prepare(" AND `item_cat` = %s", $category);
				if(trim($enabled ?? '') != '') $condition .= wpl_db::prepare(" AND `enabled` >= %s", $enabled);

				$condition .= " ORDER BY `index` ASC";
			}

			$records = wpl_db::select("SELECT * FROM `#__wpl_items` WHERE 1 ".$condition);
		}
	...
	public static function update($item_id, $values = array())
	{
		...
		$affected_rows = wpl_db::update('wpl_items', $values, 'id', $item_id);
	...
	public static function insert($values = array())
	{
		...
		$insert_id = wpl_db::insert('wpl_items', $values);
	...
	public static function delete_all_items($parent_id, $kind = 0)
	{
		...
		/** delete items **/
		return wpl_db::q(wpl_db::prepare("DELETE FROM `#__wpl_items` WHERE `parent_kind` = %d AND `parent_id` = %d", $kind, $parent_id));
	}
	...
	public static function update_file($file_name, $parent_id, $values = array())
	{
		...
		foreach($values as $key=>$value) $q .= wpl_db::prepare('%i = %s, ', $key, $value);
	...
	public static function get_item_categories($item_type, $parent_kind = 0, $condition = '')
	{
		...
		$records = wpl_db::select("SELECT * FROM `#__wpl_item_categories` WHERE 1 ".$condition);
    ...
    {
        return wpl_db::select(wpl_db::prepare("SELECT * FROM `#__wpl_item_categories` WHERE 1 AND `id` = %d", $id), 'loadObject');
    }
	...
	public static function get_maximum_index($parent_id, $item_type = '', $parent_kind = 0, $category = '', $enabled = NULL, $condition = '')
	{
		...
		$index = wpl_db::select("SELECT MAX(`index`) as max FROM `#__wpl_items` WHERE 1 ".$condition,'loadObject');
		return $index->max;
	}
	...
	public static function render_gallery($images = array(), $blog_id = NULL)
	{
		...
		foreach($images as $image)
		{
			...
			// perform that filesize should be in internal items
			if($image['item_cat'] == 'internal'){
                $return[$i]['size'] = @filesize($image_path);
            }else{
                $return[$i]['size'] = $image_path;
            }
		...
		// Apply Filters
		@extract(wpl_filters::apply('wpl_render_gallery', array('return'=>$return)));
	...
	public static function render_attachments($attachments = array(), $blog_id = NULL)
	{
		...
		// Apply Filters
		@extract(wpl_filters::apply('wpl_render_attachments', array('return'=>$return)));
	...
	public static function render_videos($videos = array(), $blog_id = NULL)
	{
		...
		foreach($videos as $video)
		{
			...
			if($video['item_cat'] == 'video')
			{
				...
				$return[$i]['date'] = $video['creation_date'];
		...
		// Apply Filters
		@extract(wpl_filters::apply('wpl_render_videos', array('return'=>$return)));
    ...
    public static function render_opendates($open_dates = array(), $valid = true)
	{
        ...
        // Apply Filters
		@extract(wpl_filters::apply('wpl_render_opendates', array('opendates'=>$opendates)));
    ...
    public static function render_rooms($rooms = array())
	{
        ...
        // Apply Filters
		@extract(wpl_filters::apply('wpl_render_rooms', array('rendered'=>$rendered)));
    ...
    public static function clone_items($source_id, $destination_id)
    {
        ...
        {
			wpl_db::insert('wpl_items', [
				'parent_kind' => $destination_kind,
				...
				'item_extra1' => $item->item_extra1,
				'item_extra2' => $item->item_extra2,
				'item_extra3' => $item->item_extra3,
				'item_extra4' => $item->item_extra4,
				'item_extra5' => $item->item_extra5,
				'params' => $item->params,
				'index' => $item->index,
			]);
        }
