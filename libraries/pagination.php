...
class wpl_pagination
{
    ...
    public function show()
	{
		...
		{
			wpl_esc::e('<ul class="' . wpl_esc::return_attr($this->className) . '">' . $this->pagination . '</ul>');

            if(isset($this->show_total) and $this->show_total) wpl_esc::e( '<span class="wpl_total_result">' . intval($this->total_pages) . ' ' . wpl_esc::return_html_t('Results returned.') . '</span>');
            if(isset($this->show_page_size) and $this->show_page_size)
			{
                $page_sizes = explode(',', trim(wpl_global::get_setting('page_sizes') ?? '', ', '));

                wpl_esc::e('<span class="wpl_page_size">');
                wpl_esc::e('<span class="wpl_page_size_title">' . wpl_esc::return_html_t('Per Page ') . '</span>');
                wpl_esc::e('<select class="wpl_page_size_options" onchange="wpl_pagesize_changed(this.value);">');
				
                foreach ($page_sizes as $page_size) wpl_esc::e('<option value="' . intval($page_size) . '" ' . ($this->limit == $page_size ? 'selected="selected"' : '') . '>' . intval($page_size) . '</option>');
    ...
    public function getOutput()
	{
        ...
        if(!$this->calculate and $this->calculate())
		{
			$string .= '<ul class="' . wpl_esc::return_attr($this->className) . '">' . $this->pagination . '</ul>';

            if($this->show_total) $string .= '<span class="wpl_total_result">' . intval($this->total_pages) . ' ' . wpl_esc::return_html_t('Results returned.') . '</span>';
            if($this->show_page_size)
			{
                $page_sizes = explode(',', trim(wpl_global::get_setting('page_sizes') ?? '', ', '));

                $string .= '<span class="wpl_page_size">';
                $string .= '<span class="wpl_page_size_title">' . wpl_esc::return_html_t('Per Page ') . '</span>';
                $string .= '<select class="wpl_page_size_options" onchange="wpl_pagesize_changed(this.value);">';
				
                foreach($page_sizes as $page_size) $string .= '<option value="' . intval($page_size) . '" ' . ($this->limit == $page_size ? 'selected="selected"' : '') . '>' . intval($page_size) . '</option>';
    ...
    public static function get_pagination($num_result, $page_size = '', $show_options = false, $js_link = 0)
	{
        ...
        $p->nextLabel(wpl_esc::return_html_t("Next"));
        $p->prevLabel(wpl_esc::return_html_t("Previous"));
