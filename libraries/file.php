...
class wpl_file
{
     ...
     * @return mixed
     */
    public static function size($file, $rendered = false)
    {
        $bytes = @filesize($file);
        if(!$bytes) return 0;
        ...
        if($object->hasChildNodes())
        {
            ...
            if($object->childNodes->length == 1 and $is_array == 0)
            {
                if($object->nodeType != XML_TEXT_NODE)
                {
                    ...
                    {
                        $childs = array();
                        $childs[$path] = '['.$object->nodeValue.']';

                        foreach($object->attributes as $attr)
                        {
                            if(!isset($childs[$path.':'.$attr->nodeName])) $childs[$path.':'.$attr->nodeName] = '['.$attr->nodeValue.']';
                            else $childs[$path.':'.$attr->nodeName] .= '['.$attr->nodeValue.']';
                            
                            if(!isset($this->values[$path.':'.$attr->nodeName])) $this->values[$path.':'.$attr->nodeName] = '['.$attr->nodeValue.']';
                            else $this->values[$path.':'.$attr->nodeName] .= '['.$attr->nodeValue.']';
                        }
                    }
