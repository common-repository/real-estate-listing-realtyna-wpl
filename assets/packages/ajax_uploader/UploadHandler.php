...
class wpl_UploadHandler
{
    ...
    protected function get_upload_path($file_name = null, $version = null)
    {
        ...
        if (empty($version)) {
            ...
            if ($version_dir) {
                return $version_dir.$this->get_folder_path().$file_name;
            }
            $version_path = $version.'/';
        }
        return $this->options['upload_dir'].$this->get_folder_path()
    ...
    protected function get_download_url($file_name, $version = null, $direct = false)
    {
        ...
        if (empty($version)) {
            ...
            if ($version_url) {
                return $version_url.$this->get_folder_path().rawurlencode($file_name);
            }
            $version_path = rawurlencode($version).'/';
        }
        return $this->options['upload_url'].$this->get_folder_path()
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error, $index = null, $content_range = null)
    {
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if (function_exists('mime_content_type')) {
                $current_filetype = mime_content_type($uploaded_file);
                $mime = explode('/',$current_filetype);
                if ($mime[0]=='image') {
                    switch($mime[1]) {
                    ...
                    if (function_exists('pathinfo')) {
                        ...
                        $file->name = $info['filename'].$ext;
                    } else {
                        $info = explode('.',$file->name);
                        array_pop($info);
                        array_push($info,'txt');
                        $file->name = implode('.',$info);
                    }
                }
            }
    ...
    protected function readfile($file_path)
    {
        ...
        if ($chunk_size && $file_size > $chunk_size) {
            ...
            while (!feof($handle)) { 
                wpl_esc::e(fread($handle, $chunk_size));
                ob_flush(); 
                flush(); 
            } 
            fclose($handle); 
