...
// get img string base64
function wpl_get_base64_image(img) {var canvas = document.createElement("canvas");canvas.width = img.width;canvas.height = img.height;var ctx = canvas.getContext("2d");ctx.drawImage(img, 0, 0);var dataURL = canvas.toDataURL("image/png");return dataURL;}

// checks if the browser is compatible with snapshot 
function wpl_is_proper_device() {var window_width = jQuery(window).width(); var window_height = jQuery(window).height(); if(window_width < 1200) return false; if(window_height < 400) return false;var ua= navigator.userAgent, tem, M= ua.match(/(opera|chrome|safari|firefox|msie|trident(?=\/))\/?\s*(\d+)/i) || [];if(/trident/i.test(M[1])){tem=  /\brv[ :]+(\d+)/g.exec(ua) || [];}if(M[1]=== 'Chrome'){tem= ua.match(/\b(OPR|Edge)\/(\d+)/);if(tem!= null){tem.slice(1).join(' ').replace('OPR', 'Opera');}}M= M[2]? [M[1], M[2]]: [navigator.appName, navigator.appVersion, '-?']; if((tem= ua.match(/version\/(\d+)/i))!= null) M.splice(1, 1, tem[1]); var browser = M[0]; var browser_version = M[1]; if(browser.toLowerCase() === 'chrome' && parseInt(browser_version) > 70){return true;} else {return false;}}