( function ( $ ) {
    $( document ).on( 'widget-added', function ( event, widget ) {
        console.log(widget);
        jQuery.each(jQuery(widget).parent().parent().find('[id ^=widget-wpl_search_widget-][id $=wpl_search_widget_container]'),function(i,v){


            var widget_number = jQuery(this).attr('id').replace('widget-wpl_search_widget-','').replace('wpl_search_widget_container','').replace('-','');

            wplj(function(){
                // - Bind click event to View fields button
                wplj('#btn-search-'+widget_number).off('click').on('click',function(evn){
                    evn.preventDefault();
                    evn.stopPropagation();
                    ...
                    wplj._realtyna.lightbox.open(wplj(this), {
                        ...
                        callbacks: {
                            beforeOpen: function(){
                                wplj('#wpl_flex_modify_container_'+widget_number+' .wpl-widget-search-fields-wp').css({
                                    maxWidth: '100%',
                                    minWidth: width,
                                    width   : width,
                                    height  : height
                                ...
                                if(Realtyna.getBrowserSizes().browser().width > 1400){
                                    wplj('#wpl_flex_modify_container_'+widget_number).addClass('wpl-widget-search-4-col');
                                }
                            ...
                            afterOpen : function(){
                                ...
                                wplj('#wpl_view_fields_'+widget_number+' .fancy-search-body > div').height(height);
            
                                // - Set Active block height
                                var fields = wplj('#wpl_flex_modify_container_'+widget_number+' .search-fields-wp'),
                                    fieldsHeight = fields.outerHeight(),
                                    contentWidth = fields.outerWidth(),
                                    contentHeight = fieldsHeight - ( 39 *  window['wplSearchWidget'+widget_number].tabRows),
                                ...
                                // - Set OrderList block
                                var orderList = wplj('#wpl_flex_modify_container_'+widget_number+' .order-list-wp'),
                                    listHeight = orderList.outerHeight() - 39;
            
                                orderList.find('.order-list-body').css('height', listHeight);
            
                                // - Initialize scrollbars
                                window['wplSearchWidget'+widget_number].setScrolls();
                                window['wplSearchWidget'+widget_number].addDroppableOverlays();
                                window['wplSearchWidget'+widget_number].resizeInactiveBlock();
                            },
                            afterClose : function(){
                                wplj(".wpl-widget-row input[type=text][id^='widget-wpl_search_widget-'][id$='-title']").trigger('change');
                            }
                        }
                    });
            ...
            window['wplSearchWidget'+widget_number] = (function ($) {
                ...
                W.afterOpened = function(){
                    $('#wpl_view_fields_'+widget_number+' .fancy-search-body > div').equalHeight();
                }
                ...
                W.saveAndReload = function(that){
                    ...
                    $(document).ajaxComplete(function(){
                        $('#btn-search-'+widget_number).off('click').on('click',function(){
                ...
                W.setScrolls = function(){
                    ...
                    $('#wpl_view_fields_'+widget_number + selActive).mCustomScrollbar({
                        theme: "minimal-dark",
                        scrollInertia: 300,
                        autoHideScrollbar: 1
                    });
                    ...
                    jQuery('#wpl_view_fields_'+widget_number+' .wpl-inactive-block-wp').each(function(){
                    ...
                    // - Orderlist scroll
                    $('#wpl_view_fields_'+widget_number+' .order-list-body').mCustomScrollbar({
                        theme: "minimal-dark",
                        scrollInertia: 300
                    });
                ...
                W.addDroppableOverlays = function(){
                    var dragOverlayTmpl = '<div class="overlay-wp"><div class="overlay-text">Drag Here</div></div>';
                    $('#wpl_view_fields_'+widget_number + selActive).prepend(dragOverlayTmpl);
                }
            
                W.resizeInactiveBlock = function(tabs){
            
                    $('#wpl_view_fields_'+widget_number+' .inactive-block').each(function(){
                        var inactive = $(this),
                ...
                function getBlockId(type) {
                    var currentTab = $('#wpl_view_fields_'+widget_number).attr('data-active-tab');
                ...
                function updateOrderList(baseOnOrderList) {
                    ...
                    // Fill list of order
                    $('#wpl_view_fields_'+widget_number+' .order-list-body li').each(function (index) {
                    ...
                    for(var i=1; i <= Object.keys(orderOfItems).length; i++) {
                        ...
                        var orderItem = $('#wpl_view_fields_'+widget_number+' .order-list-body li[data-field-id='+orderOfItems[i]+']'),
                            orderItemParent = orderItem.parent();
                        orderItem.detach().appendTo(orderItemParent);
            
                        // Update order value of each item
                        $('#wpl_view_fields_'+widget_number + selActive).find('#field_sort_' + orderOfItems[i]).val(i);
            
                        // Apply reordering on Active boxes element
                        var mainItem = $('#wpl_view_fields_'+widget_number + selActive).find('.search-field-wp[data-field-id='+orderOfItems[i]+']'),
                ...
                function addToOrderList(elm, isOnInit){
                    ...
                    var order   = isOnInit ? elm.find('#field_sort_' + id).val() :
                                            $('#wpl_view_fields_'+widget_number + ' .order-list-body li:last').data('field-order') + 1 || 1,
                        item = $('<li data-field-id="' + id + '" data-field-order="' + order + '"><i>' + order + '</i><span>' + name + '</span></li>');
            
                    item.hide().appendTo('#wpl_view_fields_'+widget_number+' .order-list-wp ul');
            
                    item.fadeIn(300,function(){
                        jQuery('#wpl_view_fields_'+widget_number+' .order-list-body').mCustomScrollbar('scrollTo', 'bottom');
                    });
                ...
                function removeFromOrderList(id){
                    $('#wpl_view_fields_'+widget_number+' .order-list-wp ul').find('li[data-field-id="' + id + '"]').fadeOut(function () {
                        $(this).remove();
                ...
                $(function () {
                    ...
                    // + InitChosen
                    window['isChosenInit'+widget_number] = false;
                    ...
                    $('#wpl_view_fields_'+widget_number+' .search-msg-btn').on('click',function(){
                    ...
                    $('#wpl_view_fields_'+widget_number+' .search-field-wp').filter('[data-status="enable"]').each(function () {
                    ...
                    // - Add icons
                    $('#wpl_view_fields_'+widget_number+' .search-field-wp[data-status="disable"] h4').append(searchConfig.templates.move);
                    $('#wpl_view_fields_'+widget_number+' .search-field-wp[data-status="enable"] h4').append(searchConfig.templates.delete);
                    ...
                    if ($('#wpl_view_fields_'+widget_number + selActive + ' .search-field-wp[data-status="enable"]').length > 0) {
            
                        $('#wpl_view_fields_'+widget_number + selActive + ' .search-field-wp[data-status="enable"]').each(function () {
                    ...
                    $('#wpl_view_fields_'+widget_number+' .icon-disabled').on('click.wpl-events', function(e){ removeButtonHandler($(this), e); });
                    ...
                    // - Calc tabs size
                    var __currentTab = 0,
                        __numberOfTabs = $('#wpl_view_fields_'+widget_number+' .search-tab').length,
                        itemPerRow = 7,
                        tabsWidth = 0,
                        rows = 0;
                    ...
                    if(__numberOfTabs >= itemPerRow){
                        ...
                        $('#wpl_view_fields_'+widget_number+' .search-tabs-wp').addClass('multi-row-tab');
                        ...
                        for (var i = 0; i < __numberOfTabs; i++) {
                            ...
                            $('#wpl_view_fields_'+widget_number + ' .search-tab').eq(i).addClass(classes);
                    ...
                    // - Tab event
                    $('#wpl_view_fields_'+widget_number+' .search-tab').on('click.wpl-events',function(e){
                        e.preventDefault();
            
                        var __tabID = $(this).attr('href');
                        // Set changes on tabs
                        $('#wpl_view_fields_'+widget_number + ' .search-tab').removeClass('active');
                        $(this).addClass('active');
            
                        // Show current tab content
                        $('#wpl_view_fields_'+widget_number + ' .search-body').hide();
            
                        // Save current tabs & Show active tab content
                        $('#wpl_view_fields_'+widget_number).attr({'data-active-tab': __tabID}).find(__tabID).fadeIn();
                    ...
                    $('#wpl_view_fields_'+widget_number).attr({'data-active-tab': $('#wpl_view_fields_'+widget_number).find('.search-tab').eq(__currentTab).attr('href')});
                    $('#wpl_view_fields_'+widget_number + ' .search-tab').css({width: tabsWidth + '%'}).eq(__currentTab).trigger('click');
                    ...
                    */
                    ...
                    $('#wpl_view_fields_'+widget_number+' #fields-order ul').sortable({
                        handle: 'i',
                        opacity: 0.5,
                        ...
                        change: function (event, ui) {
                            var p = ui.position.top,
                                ...
                                elem = $('#wpl_view_fields_'+widget_number + ' .order-list-body .mCustomScrollBox')[0],
                                elemHeight = $('#wpl_view_fields_'+widget_number + ' .order-list-body .mCustomScrollBox').height(),
                                pos = findPos(elem),
                                mouseCoordsY = event.pageY - pos[0];
            
                            if (mouseCoordsY < h || mouseCoordsY > elemHeight - h) {
                                $('#wpl_view_fields_'+widget_number + ' .order-list-body').mCustomScrollbar('scrollTo', p - (elemHeight / 2));
                            }
                        }
                    });
            
            
                    $('#wpl_view_fields_'+widget_number+' .order-list-wp ul').disableSelection();
            
            
                    // - Initialize InactiveBlock for draggable
                    $('#wpl_view_fields_'+widget_number + selInactive + ' .search-field-wp').draggable({
                        cursor: 'move',
                        revert: true,
                        delay: 200,
                    ...
                    $('#wpl_view_fields_'+widget_number + selActive).droppable({
                    ...
                    $('#wpl_view_fields_'+widget_number+' .overlay-wp').each(function () {
                    ...
                    $('#btn-search-'+widget_number).addClass('fancybox').attr('data-is-init', 'true');
                    $('#btn-shortcode-'+widget_number).addClass('fancybox').attr('data-is-init', 'true');
                    $('#wpl-js-page-must-reload-'+widget_number).hide();
