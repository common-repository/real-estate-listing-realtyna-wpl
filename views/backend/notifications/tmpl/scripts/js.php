...
function wpl_set_enabled_notification(notification_id, enabled_status, enabled_field)
{
	if (!notification_id)
	{
		wpl_show_messages("<?php wpl_esc::js_t('Invalid Notification'); ?>", '.wpl_notification_list .wpl_show_message');
		return false;
	}
	...
	request_str = 'wpl_format=b:notifications:ajax&wpl_function=set_enabled_notification&notification_id=' + notification_id + '&enabled_status=' + enabled_status + '&_wpnonce=<?php wpl_esc::attr($this->nonce); ?>&enabled_field=' + enabled_field;
	wplj.ajax({
		type: 'POST',
		dataType: 'JSON',
