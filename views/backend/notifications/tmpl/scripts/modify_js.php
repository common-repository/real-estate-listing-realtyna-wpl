...
jQuery(document).ready(function()
{
	wplj(function()
	{
		wplj('#wpl_notification_form').submit(function(e)
		{
			...
			var request_str = 'wpl_format=b:notifications:ajax&wpl_function=save_notification&info[include_email]=' + include_email + '&info[include_membership]=' + include_membership + '&info[include_user]=' + include_user + '&' + wplj(this).serialize() + '&_wpnonce=<?php wpl_esc::attr($this->nonce); ?>&'+sms_request_str;
			wplj.ajax({
				type: 'POST',
				dataType: 'HTML',
				...
				success: function (data) {
					...
					wpl_show_messages('<?php wpl_esc::js_t('Notification modified succesfully.'); ?>', '.wpl_notification_modify .wpl_show_message', 'wpl_green_msg');
				},
...
function add_recipients(sel,inc,field)
{
	...
	if(field == 'email_recipients')
	{
		...
		{
			wpl_alert("<?php wpl_esc::js_t("Please enter a valid email address"); ?>");
			return;
		}
	...
	else
	{
		...
		{
			wpl_alert("<?php wpl_esc::js_t("Please select at least one option"); ?>");
			return;
		}
