...
class wpl_addon_idx_controller extends wpl_controller
{
    ...
    protected static function configuration(addon_idxn $idxClient)
    {
        ...
        if($current_user->user_email)
        {
            $formparams = array(
                ...
                'download_images' => wpl_request::getVar('download_images')
            );
 ...
 public static function save_client_request (addon_idxn $idxClient) 
 {
   ...
   // Using this method we will cancel the client's subscription
  protected static function cancel_subscription( addon_idxn $idxClient)
  {
      ...
      if(!$current_user->user_email)
      {
       ...
       wp_send_json($idxClient->cancel_subscription());
