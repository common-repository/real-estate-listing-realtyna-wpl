...
class wpl_activity_manager_controller extends wpl_controller
{
    ...
    public function display()
    {
        ...
        // Check Nonce
        if(!wpl_security::verify_nonce(wpl_request::getVar('_wpnonce', ''), 'wpl_activity_manager')) $this->response(array('success'=>0, 'message'=>wpl_esc::return_html_t('The security nonce is not valid!')));
    ...
    {
        wpl_esc::e(wpl_activity::sort($sort_ids));
    }
    ...
    private function remove_activity($activity_id, $confirmed = 0)
    {
        ...
        $message = $res ? wpl_esc::return_html_t('Activity removed successfully.') : wpl_esc::return_html_t('Error Occured.');
        $data = NULL;
    ...
    private function generate_modify_page()
    {
        ...
        {
            $this->activity_data = wpl_activity::get_activity(wpl_db::prepare('AND `id`=%d', $this->activity_id));
    ...
    {
        $current_activity = wpl_activity::get_activity(wpl_db::prepare('AND `activity` = %s', $activity_name));
        ...
        else $returnData['options'] = wpl_esc::return_html_t("This activity doesn't have options!");
