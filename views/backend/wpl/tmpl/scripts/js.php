...
function check_addon_update(addon_id)
{
	...
	/** run ajax query **/
	var request_str = 'wpl_format=b:wpl:ajax&wpl_function=check_addon_update&addon_id='+addon_id+'&_wpnonce=<?php wpl_esc::attr($nonce); ?>';
	wplj.ajax({
		type: 'POST',
		dataType: 'JSON',
...
function update_package(sid)
{
	...
	/** run ajax query **/
	var request_str = 'wpl_format=b:wpl:ajax&wpl_function=update_package&sid=' + sid + '&_wpnonce=<?php wpl_esc::attr($nonce); ?>';
	wplj.ajax({
		type: 'POST',
		dataType: 'JSON',
		url: '<?php wpl_esc::current_url(); ?>',
...
function save_realtyna_credentials()
{
	...
	/** run ajax query **/
	var request_str = 'wpl_format=b:wpl:ajax&wpl_function=save_realtyna_credentials&username='+username+'&password='+password+'&_wpnonce=<?php wpl_esc::attr($nonce); ?>';
	wplj.ajax({
		type: 'POST',
		dataType: 'JSON',
		url: '<?php wpl_esc::current_url(); ?>',
...
function check_envato_purchase_code(type,fullname,email,purchase)
{
	if (type == 'submit') 
	{
		...
		var request_str = 'wpl_format=b:wpl:ajax&wpl_function=check_envato_purchase_code&type='+type+'&fullname='+fullname+'&email='+email+'&purchase='+purchase+'&_wpnonce=<?php wpl_esc::attr($nonce); ?>';
	}
	else
	{
		var request_str = 'wpl_format=b:wpl:ajax&wpl_function=check_envato_purchase_code&type='+type+'&_wpnonce=<?php wpl_esc::attr($nonce); ?>';
	}
