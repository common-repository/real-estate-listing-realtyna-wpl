...
jQuery(document).ready(function()
{
	...
	wplj('.wpl_prp_show_tabs ul.tabs li a').off('touchstart click').on('touchstart click',function()
	{
		...
		if(init_google_map && typeof wpl_pshow_map_init<?php wpl_esc::numeric($this->pshow_googlemap_activity_id); ?> == 'function')
		{
			wpl_pshow_map_init<?php wpl_esc::numeric($this->pshow_googlemap_activity_id); ?>();
		}
		<?php endif; ?>
        
        <?php if(isset($this->pshow_bingmap_activity_id)): ?>
        var init_bing_map = wplj(this).attr('data-init-bingmap');
		if(init_bing_map && typeof wpl_pshow_map_init<?php wpl_esc::numeric($this->pshow_bingmap_activity_id); ?> == 'function')
		{
			wpl_pshow_map_init<?php wpl_esc::numeric($this->pshow_bingmap_activity_id); ?>();
		}
		<?php endif; ?>
			...
			wplj._realtyna.lightbox.open("#wpl_dpr_lightbox",
				{
					...
					closeOnOverlay: <?php wpl_esc::E(wpl_session::get('wpl_dpr_popup') == 1 ? 'true' : 'false'); ?>,
					callbacks:
						{
							afterClose: function()
							{
...
function wpl_listhub_tracker()
{
	...
	<?php wpl_esc::e(wpl_addon_listhub::lishub_metrics_js()); ?>
	
	lh('init', {provider:'<?php wpl_esc::js($this->settings['listhub_tracking_metrics_id']); ?>', test:false});
	lh('submit', 'DETAIL_PAGE_VIEWED', {lkey:'<?php wpl_esc::js($this->wpl_properties['current']['raw']['listing_key']); ?>'});
}
<?php endif; ?>
