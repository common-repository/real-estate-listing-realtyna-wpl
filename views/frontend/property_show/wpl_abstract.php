...
abstract class wpl_property_show_controller_abstract extends wpl_controller
{
	...
	public function display($instance = array())
	{
        ...
        {
            $this->message = wpl_esc::return_html_t("You reached the maximum daily visit Today. You can browse listings Tomorrow again!");
            return parent::render($this->tpl_path, 'message', false, true);
        }
		...
		$rlistings = !empty($current_user->maccess_listings_pshow) ? explode(',', $current_user->maccess_listings_pshow) : '';
		$ptrestrict = !empty($current_user->maccess_ptrestrict_pshow) ? $current_user->maccess_ptrestrict_pshow : '';
		$rproperty_types = !empty($current_user->maccess_property_types_pshow) ? explode(',', $current_user->maccess_property_types_pshow) : '';

		if(($lrestrict and !in_array($property['listing'], $rlistings)) or ($ptrestrict and !in_array($property['property_type'], $rproperty_types)))
		{
			$this->message = wpl_esc::return_html_t("Sorry! You don't have access to view this property.");
            
			return parent::render($this->tpl_path, 'message', false, true);
		}
        
		$this->pshow_fields = $this->model->get_pshow_fields('', $property['kind']);
		$this->pshow_categories = wpl_flex::get_categories('', '', wpl_db::prepare(" AND `enabled`>='1' AND `kind` = %d AND `pshow`='1'", $property['kind']));
