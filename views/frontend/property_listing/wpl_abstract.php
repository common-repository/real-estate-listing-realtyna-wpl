...
abstract class wpl_property_listing_controller_abstract extends wpl_controller
{
    ...
    public function display($instance = array())
    {
        ...
        {
            // Import Message tpl
            if(wpl_users::is_administrator()) $this->message = sprintf(wpl_esc::return_html_t("You don't have access to this menu! %s KB article might be helpful."), '<a href="https://support.realtyna.com/index.php?/Default/Knowledgebase/Article/View/618/">'.wpl_esc::return_html_t('this').'</a>');
            else
            {
                $this->message = wpl_esc::return_html_t("You don't have access to see properties! Please login or register first.");
                $this->error_code = 401;
            }
        ...
        $this->method = wpl_request::getVar('wplmethod');
		...
		$this->page_number = intval($this->page_number);
        ...
        // RSS Feed Setting
        $this->listings_rss_enabled = $this->settings['listings_rss_enabled'] ?? 0;

        // Print Results Page
        $this->print_results_page = $this->settings['pdf_results_page_status'] ?? 0;

        // Save Search Button
        $this->save_search_button = $this->settings['ss_button_status'] ?? 1;
        ...
        $this->label_agent_name = $this->settings['label_agent_name'] ?? "";
        $this->label_office_name = $this->settings['label_listing_brokerage'] ?? "";

        // Favorite btn show or hide
        $this->favorite_btn = $this->settings['wpl_ui_customizer_property_listing_favorite_btn'] ?? 1;

        // Realtyna Signature and Affiliate
        $this->show_signature = $this->settings['realtyna_signature'] ?? 1;
        $this->affiliate_id = ( trim($this->settings['realtyna_affiliate_id'] ?? '' ) ) ? $this->settings['realtyna_affiliate_id'] : 4;
        ...
        {
            // Import Message TPL
            $this->message = wpl_esc::return_html_t('Invalid Request!');
            return parent::render($this->tpl_path, 'message', false, true);
        }
