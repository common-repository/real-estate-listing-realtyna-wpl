...
class wpl_request_controller
{
	...
	public function run()
	{
		if(class_exists($this->_wpl_class)) {
			...
			/*
			 * David.M
			 * I added this hook to move override files to /customization/hooks
			 * Added after display function, because we should not overwrite the main function
			 * and the function should be specified
			 */
			$function = wpl_request::getVar('wpl_function');
			if(!empty($function)) {
				do_action("wpl_request_controller/run/" . $this->_wpl_request_format . '/' . $function, $_wpl_request_obj);
			}
		} else {
			do_action("wpl_request_controller/run/" . $this->_wpl_request_format);
		}
		/** exit **/
		exit;
	}
...
if( !empty($wpl_format) )
{
	if(wpl_global::check_addon('pro')) {
		_wpl_import('addons.addon_loader');
		WplAddonLoader::getInstance()->load();
	}
    ...
    else $hook = 'wp'; # WordPress Frontend
