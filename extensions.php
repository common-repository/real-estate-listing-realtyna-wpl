...
class wpl_extensions
{
    ...
    {
		$extension = apply_filters('wpl_extensions/import_style', $extension);
        /** render style_url **/
    ...
    {
        $name = ( trim($extension->title ?? '' ) ) ? $extension->title : 'WPL sidebar';
        $id = ( trim($extension->param1 ?? '' ) ) ? $extension->param1 : 'wpl-sidebar-id';
        $description = ( trim($extension->description ?? '' ) ) ? $extension->description : 'WPL sidebar description';
        $before_widget = ( trim($extension->param2 ?? '' ) ) ? $extension->param2 : '<aside id="%1$s" class="widget %2$s">';
        $after_widget = ( trim($extension->param3 ?? '' ) ) ? $extension->param3 : '</aside>';
        $before_title = ( trim($extension->param4 ?? '' ) ) ? $extension->param4 : '<h3 class="widget-title">';
	...
	private function runQueries($file) {
		if(wpl_file::exists($file)) {
			$read_file = wpl_file::read($file);
			if($read_file != '')
			{
	...
	private function migrate($path, $version_key) {
		...
		if(wpl_folder::exists($queries)) {
			...
			$files = wpl_folder::files($queries);
			...
			foreach($files as $file) {
				...
				if(version_compare($version, $installed_version, '>')) {
					...
					if($fileType == 'php') {
						...
						$this->runQueries($filePath);
					}
					update_option($version_key, $version);
				}
			}
			if($version_key == 'wpl_pro_migration') {
				// UPDATE WPL PRO version in Database
				wpl_db::update('wpl_addons', ['version' => WPL_VERSION], 'id', 3);
			}
		}
	}
	private function includeOnce($filePath) {
		include_once $filePath;
	}

	public function migration() {
		/** run basic queries **/
		$this->migrate(WPL_ABSPATH. 'assets' .DS. 'migrations' .DS. 'basic' . DS, 'wpl_basic_migration');

		/** run pro queries **/
		$this->migrate(WPL_ABSPATH. 'assets' .DS. 'migrations' .DS. 'pro' . DS, 'wpl_pro_migration');
	}
    ...
    public function activate_wpl($network_activate = false)
    {
        ...
        /** set cron to check license */
        if ( ! wp_next_scheduled( 'rlty_check_user_license' ) ) {
            wp_schedule_event(time(), 'weekly', 'rlty_check_user_license');
        }
    }
    ...
    public static function import_styles_scripts()
    {
		...
		/**
		 * to fix conflicts with avia Google map and our Google map
		 */
		if(wpl_global::get_client()) {
    ...
    public static function plugin_action_links($links)
    {
		if(wpl_global::check_addon('pro') === false) {
			$links[] = '<a class="pro-link" href="https://realtyna.com/wpl-platform/ref/312/?utm_source=wpl-backend&utm_medium=link&utm_campaign=WplBackend" target="_blank">'.__('Get WPL PRO', 'real-estate-listing-realtyna-wpl').'</a>';
		}
    ...
    /**
     * Block editor assets
     * @author Leo I. <leo@realtyna.com>
     * @return void
     */
    public static function import_block_editor_scripts()
    {
        wp_add_inline_script( 'wp-widgets',
            file_get_contents('assets/js/widget_block_editor.js',true)
        );
    }

	/**
	 * Check wpl_properties table engine is innodb or not
	 *
	 ...
	 * @date 13/01/2023
	 */
	public function add_wpl_innodb_notice() {
		$propertiesTableDetails = wpl_db::select( "SHOW TABLE STATUS WHERE Name = '#__wpl_properties'" , 'loadAssoc' );
		$propertiesTableEngine = !empty( $propertiesTableDetails['Engine'] ) ? $propertiesTableDetails['Engine'] : 'Unknown';
...
add_filter( 'plugin_action_links_' . WPL_PLUGIN_BASE, [ $wpl_extensions, 'plugin_action_links' ] );
