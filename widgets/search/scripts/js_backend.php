...
wplj(function(){
    ...
    wplj('#btn-search-<?php wpl_esc::numeric($this->number); ?>').off('click').on('click',function(evn){
        evn.preventDefault();
        evn.stopPropagation();
        ...
        wplj._realtyna.lightbox.open(wplj(this), {
            ...
            callbacks: {
                beforeOpen: function(){
                    wplj('#wpl_flex_modify_container_<?php wpl_esc::numeric($this->number); ?> .wpl-widget-search-fields-wp').css({
                        maxWidth: '100%',
                        minWidth: width,
                        width   : width,
                        height  : height
                    ...
                    if(Realtyna.getBrowserSizes().browser().width > 1400){
                        wplj('#wpl_flex_modify_container_<?php wpl_esc::numeric($this->number); ?>').addClass('wpl-widget-search-4-col');
                    }

                },
                afterOpen : function(){
                    // Init some item in search widget
                    //wplSearchWidget<?php wpl_esc::numeric($this->number); ?>.afterOpened();

                    // + Set content heights base on new sizes

                    wplj('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .fancy-search-body > div').height(height);

                    // - Set Active block height
                    var fields = wplj('#wpl_flex_modify_container_<?php wpl_esc::numeric($this->number); ?> .search-fields-wp'),
                        fieldsHeight = fields.outerHeight(),
                        contentWidth = fields.outerWidth(),
                        contentHeight = fieldsHeight - ( 39 *  wplSearchWidget<?php wpl_esc::numeric($this->number); ?>.tabRows),
                    ...
                    // - Set OrderList block
                    var orderList = wplj('#wpl_flex_modify_container_<?php wpl_esc::numeric($this->number); ?> .order-list-wp'),
                        listHeight = orderList.outerHeight() - 39;

                    orderList.find('.order-list-body').css('height', listHeight);

                    // - Initialize scrollbars
                    wplSearchWidget<?php wpl_esc::numeric($this->number); ?>.setScrolls();
                    wplSearchWidget<?php wpl_esc::numeric($this->number); ?>.addDroppableOverlays();
                    wplSearchWidget<?php wpl_esc::numeric($this->number); ?>.resizeInactiveBlock();
                },
                afterClose : function(){
                    wplj("#<?php wpl_esc::attr($this->get_field_id('css_class')); ?>").trigger('change');
                }
            }
        });
...
var wplSearchWidget<?php wpl_esc::numeric($this->number); ?> = (function ($) {
    ...
    W.afterOpened = function(){
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .fancy-search-body > div').equalHeight();
    }
    ...
    W.saveAndReload = function(that){
        ...
        $(document).ajaxComplete(function(){
            $('#btn-search-<?php wpl_esc::numeric($this->number); ?>').off('click').on('click',function(){
    ...
    W.setScrolls = function(){
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive).mCustomScrollbar({
            theme: "minimal-dark",
            scrollInertia: 300,
            autoHideScrollbar: 1
        });
        ...
        jQuery('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .wpl-inactive-block-wp').each(function(){
        ...
        // - Orderlist scroll
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-body').mCustomScrollbar({
            theme: "minimal-dark",
            scrollInertia: 300
        });
    ...
    W.addDroppableOverlays = function(){
        var dragOverlayTmpl = '<div class="overlay-wp"><div class="overlay-text"><?php wpl_esc::html_t('Drag Here'); ?></div></div>';
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive).prepend(dragOverlayTmpl);
    }

    W.resizeInactiveBlock = function(tabs){

        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .inactive-block').each(function(){
            var inactive = $(this),
    ...
    function getBlockId(type) {
        var currentTab = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').attr('data-active-tab');
    ...
    function updateOrderList(baseOnOrderList) {
        ...
        // Fill list of order
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-body li').each(function (index) {
        ...
        for(var i=1; i <= Object.keys(orderOfItems).length; i++) {
            ...
            var orderItem = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-body li[data-field-id='+orderOfItems[i]+']'),
                orderItemParent = orderItem.parent();
            orderItem.detach().appendTo(orderItemParent);

            // Update order value of each item
            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive).find('#field_sort_' + orderOfItems[i]).val(i);

            // Apply reordering on Active boxes element
            var mainItem = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive).find('.search-field-wp[data-field-id='+orderOfItems[i]+']'),
    ...
    function addToOrderList(elm, isOnInit){
        ...
        var order   = isOnInit ? elm.find('#field_sort_' + id).val() :
                                $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .order-list-body li:last').data('field-order') + 1 || 1,
            item = $('<li data-field-id="' + id + '" data-field-order="' + order + '"><i>' + order + '</i><span>' + name + '</span></li>');

        item.hide().appendTo('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-wp ul');

        item.fadeIn(300,function(){
            jQuery('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-body').mCustomScrollbar('scrollTo', 'bottom');
        });
    ...
    function removeFromOrderList(id){
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-wp ul').find('li[data-field-id="' + id + '"]').fadeOut(function () {
            $(this).remove();
    ...
    $(function () {
        ...
        // + InitChosen
        var isChosenInit<?php wpl_esc::numeric($this->number); ?> = false;
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').parents('.widget').find('.widget-title h4').on('click.wpl-search-event',function(){

            if(!isChosenInit<?php wpl_esc::numeric($this->number); ?>){
                $(this).parents('.widget').find('select').chosen(rta.config.chosen);
                isChosenInit<?php wpl_esc::numeric($this->number); ?> = true;

                $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').parents('.widget').find('.widget-title h4').off('click.wpl-search-event');
            }
        });
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-msg-btn').on('click',function(){
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-field-wp').filter('[data-status="enable"]').each(function () {
        ...
        // - Add icons
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-field-wp[data-status="disable"] h4').append(searchConfig.templates.move);
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-field-wp[data-status="enable"] h4').append(searchConfig.templates.delete);
        ...
        if ($('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive + ' .search-field-wp[data-status="enable"]').length > 0) {

            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive + ' .search-field-wp[data-status="enable"]').each(function () {
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .icon-disabled').on('click.wpl-events', function(e){ removeButtonHandler($(this), e); });
        ...
        // - Calc tabs size
        var __currentTab = 0,
            __numberOfTabs = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-tab').length,
            itemPerRow = 7,
            tabsWidth = 0,
            rows = 0;
        ...
        if(__numberOfTabs >= itemPerRow){
            ...
            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-tabs-wp').addClass('multi-row-tab');
            ...
            for (var i = 0; i < __numberOfTabs; i++) {
                ...
                $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .search-tab').eq(i).addClass(classes);
        ...
        // - Tab event
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .search-tab').on('click.wpl-events',function(e){
            e.preventDefault();

            var __tabID = $(this).attr('href');
            // Set changes on tabs
            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .search-tab').removeClass('active');
            $(this).addClass('active');

            // Show current tab content
            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .search-body').hide();

            // Save current tabs & Show active tab content
            $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').attr({'data-active-tab': __tabID}).find(__tabID).fadeIn();
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').attr({'data-active-tab': $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>').find('.search-tab').eq(__currentTab).attr('href')});
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .search-tab').css({width: tabsWidth + '%'}).eq(__currentTab).trigger('click');
        ...
        */
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> #fields-order ul').sortable({
            handle: 'i',
            opacity: 0.5,
            ...
            change: function (event, ui) {
                var p = ui.position.top,
                    ...
                    elem = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .order-list-body .mCustomScrollBox')[0],
                    elemHeight = $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .order-list-body .mCustomScrollBox').height(),
                    pos = findPos(elem),
                    mouseCoordsY = event.pageY - pos[0];

                if (mouseCoordsY < h || mouseCoordsY > elemHeight - h) {
                    $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + ' .order-list-body').mCustomScrollbar('scrollTo', p - (elemHeight / 2));
                }
            }
        });


        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .order-list-wp ul').disableSelection();


        // - Initialize InactiveBlock for draggable
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selInactive + ' .search-field-wp').draggable({
            cursor: 'move',
            revert: true,
            delay: 200,
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?>' + selActive).droppable({
        ...
        $('#wpl_view_fields_<?php wpl_esc::numeric($this->number); ?> .overlay-wp').each(function () {
        ...
        $('#btn-search-<?php wpl_esc::numeric($this->number); ?>').addClass('fancybox').attr('data-is-init', 'true');
        $('#btn-shortcode-<?php wpl_esc::numeric($this->number); ?>').addClass('fancybox').attr('data-is-init', 'true');
        $('#wpl-js-page-must-reload-<?php wpl_esc::numeric($this->number); ?>').hide();
