...
class wpl_agents_widget extends wpl_widget
{
	...
	{
		parent::__construct('wpl_agents_widget', wpl_esc::return_html_t('(WPL) Agents'), array('description'=>wpl_esc::return_html_t('Showing specific agents.')));
	}
	...
	public function widget($args, $instance)
	{
		...
		foreach($profiles as $profile)
		{
            ...
            {
                $params['image_name'] = $wpl_profiles[$profile->id]['profile_picture']['name'] ?? '';
                $profile_picture_path = $wpl_profiles[$profile->id]['profile_picture']['path'] ?? '';
                ...
                $wpl_profiles[$profile->id]['profile_picture']['image_width'] = $instance['data']['image_width'] ?? '';
                $wpl_profiles[$profile->id]['profile_picture']['image_height'] = $instance['data']['image_height'] ?? '';
            }
            
            /** company logo **/
            if(isset($wpl_profiles[$profile->id]['company_logo']['url']))
            {
                $params['image_name'] = $wpl_profiles[$profile->id]['company_logo']['name'] ?? '';
                $company_logo_path = $wpl_profiles[$profile->id]['company_logo']['path'] ?? '';
	...
	public function form($instance)
	{
		...
		{
			$instance = array('title'=>wpl_esc::return_html_t('Featured Agents'), 'layout'=>'default.php', 'data'=>array('limit'=>'4', 'orderby'=>'p.first_name', 'order'=>'DESC', 'image_width'=>'230', 'image_height'=>'230'));
