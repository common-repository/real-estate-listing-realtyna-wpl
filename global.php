...
class wpl_global
{
     ...
     * @return mixed
     */
	public static function get_property_types($property_type_id = 0, $enabled = 1)
	{
		if( !is_numeric($property_type_id) || ( (int) $property_type_id <= 0 ) )
		{
     ...
     * @return mixed
     */
	public static function get_listings($listing_id = 0, $enabled = 1)
	{
	...
	{
		return esc_url(plugins_url('assets/'.$asset, __FILE__));
	}
     ...
     * @return string
     */
    public static function svg($image, $id = NULL, $params = array(), $path = '')
    {
        /** First validation **/
     ...
     */
    public static function load_js_template($template, $path = '')
    {
        /** First validation **/
	...
	public static function get_property_type_icons()
	{
		$icon_files = wpl_global::get_icons(WPL_ABSPATH.'assets'.DS.'img'.DS.'property_types'.DS, '.svg$');
		$icons = [];
		foreach ($icon_files as $icon_file) {
			$icons[] = [
				'icon' => $icon_file,
				'url' => wpl_global::get_wpl_asset_url('img/property_types/'.$icon_file),
			];
		}
		return apply_filters('wpl_global/get_property_type_icons', $icons);
	}
     ...
     * @return string
     */
	public static function init_tmp_folder($prefix = '')
	{
		$path = wpl_global::get_tmp_path();
		$directory = $path.'tmp_' . (!empty($prefix) ? $prefix . '_' : '') . md5(microtime(true)).DS;
     ...
     * @return array
     */
    public static function check_realtyna_credentials($username = NULL, $password = NULL)
    {
        ...
        if(!$username) $username = $settings['realtyna_username'];
        if(!$password) $password = $settings['realtyna_password'];
        ...
        $io_handler = 'https://billing.realtyna.com/io/io.php';
    ...
    public static function get_upload_abs($type = 'path')
    {
        ...
        if($type == 'path')
        {
            $dir = str_replace('//', '/', $wp_path['basedir']);
     ...
     * @static
     */
    public static function include_google_recaptcha_js()
    {
    ...
    /**
     * Include Google recaptcha Field and Library
     * @author Damon <damon@realtyna.com>
     * @static
     * @param string $option
    ...
    */
    ...
    {
        return strpos(wpl_request::getVar('action', false), 'elementor') !== false;
    }
    ...
    /**
     * Importing sly script
     * @author Leo I. <leo@realtyna.com>
     * @param string $hook
     * @return void
     */
    public static function import_sly_script($hook)
    {
        if('widgets.php' != $hook) return;
        wp_enqueue_script('wpl-sly', plugin_dir_url( __FILE__ ).'assets/js/libraries/wpl.sly.min.js', array(), '1.0');
		wp_enqueue_script('jquery-effects-core');
    }

	/**
	 * Checks if the Zap search functionality is enabled and if there are no errors with Elasticsearch.
	 * @author Mateo M. <mateo.m@realtyna.com>
	 * @return bool
	 */
	public static function zap_search_enabled()
	{
		try {
			if (!class_exists('Flare\Rush\Search')) {
				throw new Exception('Flare\Rush\Search class not found');
			}
			$search = new Flare\Rush\Search();
			$search->elastic([]);
		} catch (Elasticsearch\Common\Exceptions\NoNodesAvailableException $e) {
			add_action('admin_notices', function () {
				echo '<div class="notice notice-error"><p>Elasticsearch connection error. (Please check with your host provider that the Elasticsearch service is running on the server.)</p></div>';
			});
